...
if (!customElements.get('video-component')) {
  class Video extends HTMLElement {
    ...
    init() {
      ...
      if (this.background && this.closest('.video-section').length > 0) {
    ...
    initVimeo() {
      const initVimeoPlayer = async () => {
        ...
        this.player.on('play', () => {
          if(this.closest('.video-section').length > 0){
     ...
     */
    play() {
      if (!this.player || this.player === undefined || this.pausedByUser) return;

      if (this.type === 'youtube') {
        this.player.playVideo();
      } else {
        let isPlaying = this.player.currentTime > 0 && !this.player.paused && !this.player.ended
                    && this.player.readyState > this.player.HAVE_CURRENT_DATA;
        if (!isPlaying) {
          this.player.play();
        }
      }
     ...
     */
    pause() {
      if (!this.player || this.player === undefined) return;
